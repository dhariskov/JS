class ChristmasDinner {
    constructor(budget) {
        this.budget = budget;
        this.dishes = [];
        this.products = [];
        this.guests = {};
    }

    get budget() {
        return this._budget
    }

    set budget(value) {
        if (value < 0) {
            throw Error('The budget cannot be a negative number')
        }
        this._budget = value
        return this._budget
    }

    shopping(product) {
        const productToBuy = product[0];
        const price = product[1];
        if (price > this.budget) {
            throw Error('Not enough money to buy this product')
        }
        this.products.push(productToBuy);
        this.budget -= price
        return `You have successfully bought ${productToBuy}!`
    }

    recipes(recipe) {
        for (let p of recipe.productsList) {
            if ((this.products.indexOf(p)) === -1) {
                throw Error('We do not have this product')
            }
            let obj = {
                recipeName: recipe.recipeName,
                productsList: recipe.productsList
            }
            this.dishes.push(obj)
            return `${recipe.recipeName} has been successfully cooked!`
        }
    }

    inviteGuests(name, dish) {
        for (let el of this.dishes) {
            for (let p of el.productsList) {
                if (this.guests[name]) {
                    throw Error('This guest has already been invited')
                } else {
                    this.guests[name] = dish
                    return `You have successfully invited ${name}!`
                }
            }
        }
        throw Error('We do not have this dish')
    }

    showAttendance() {
        let result = '';
        for (let [k, v] of Object.entries(this.guests)) {
            for (let el of this.dishes) {
                if (el.recipeName === v) {
                    result += `${k} will eat ${v}, which consists of ${el.productsList.join(', ')}\n`
                }
            }
        }
        return result.slice(0,-1)
    }
}

let dinner = new ChristmasDinner(300);

dinner.shopping(['Salt', 1]);
dinner.shopping(['Beans', 3]);
dinner.shopping(['Cabbage', 4]);
dinner.shopping(['Rice', 2]);
dinner.shopping(['Savory', 1]);
dinner.shopping(['Peppers', 1]);
dinner.shopping(['Fruits', 40]);
dinner.shopping(['Honey', 10]);

dinner.recipes({
    recipeName: 'Oshav',
    productsList: ['Fruits', 'Honey']
});
dinner.recipes({
    recipeName: 'Folded cabbage leaves filled with rice',
    productsList: ['Cabbage', 'Rice', 'Salt', 'Savory']
});
dinner.recipes({
    recipeName: 'Peppers filled with beans',
    productsList: ['Beans', 'Peppers', 'Salt']
});

dinner.inviteGuests('Ivan', 'Oshav');
dinner.inviteGuests('Petar', 'Folded cabbage leaves filled with rice');
dinner.inviteGuests('Georgi', 'Peppers filled with beans');

console.log(dinner.showAttendance());
